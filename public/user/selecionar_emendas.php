<?php
// /public/user/selecionar_emendas.php
session_start();
...
// Gerar CSRF token se não existe
if (!isset($_SESSION['csrf_token'])) {
    $_SESSION['csrf_token'] = bin2hex(random_bytes(32));
}

// Função para normalizar chave para busca na planilha
...
// Função para carregar mapa de valores da planilha (fallback valor_pretendido)
...
// Função para processar valor monetário string para float
...
try {
    ...
    // Aplicar fallback valor_pretendido e calcular valor_disponivel
    foreach ($emendas as &$emenda) {
        ...
        // Calcular valor destinado somando valores_destinados
                    ...
                    <tbody>
                        <?php foreach ($emendas as $emenda): ?>
                            <tr>
                                ...
                                <td><?= htmlspecialchars(substr($emenda['objeto_intervencao'], 0, 100)) ?><?= strlen($emenda['objeto_intervencao']) > 100 ? '...' : '' ?>
                                </td>
                                ...
                                <td><?= formatarValor($emenda['valor_pretendido']) ?></td>
                                <td><?= formatarValor($emenda['valor_disponivel']) ?></td>
                                <td>
                                    <?php if (!in_array($emenda['id'], $minhas_emendas_ids)): ?>
                                        <form method="POST" style="display:inline;">
                                            ...
                                            <button type="submit" class="btn btn-primary" title="Selecionar Emenda">
                                    ...
                                    <?php else: ?>
                                        <form method="POST" style="display:inline;">
                                            ...
                                            <input type="hidden" name="emenda_id" value="<?= $emenda['id'] ?>">
                                            <button type="submit" class="btn btn-danger" title="Remover Emenda">
                                                <span class="material-icons">delete</span>
                                            </button>
                                        </form>
                                    <?php endif; ?>
                                    <a href="visualizar_emenda.php?id=<?= $emenda['id'] ?>" class="btn btn-info"
